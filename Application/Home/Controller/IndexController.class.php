...
class IndexController extends HomeController {
    ...
    public function index(){
    	/*首页项目*/
    	$projects2 = M('Project')->where("status=2 AND prrate=2")->limit("3")->select();
    	$projects3 = M('Project')->where("status=2 AND prrate=3")->limit("3")->select();
    	$projects4 = M('Project')->where("status=2 AND prrate=4")->limit("3")->select();
